...
const manHattanDistance = ({
  x1, x2, y1, y2,
   ...
   { x1: number; x2: number; y1: number; y2: number; }): number => Math.abs(x2 - x1) + Math.abs(y2 - y1);

const findClosest = (x: number, y: number, coords: Map<string, Coord>): string => {
  let minPos: Coord | null = null;
  let minDistance: number = Number.MAX_SAFE_INTEGER;
  let minDistanceCount: number = 0;
  for (const pos of coords.values()) {
    const distance = manHattanDistance({
      x1: pos.x, x2: x, y1: pos.y, y2: y,
    });
